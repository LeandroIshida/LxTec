...
<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buscar'])) 
  {
    ...
    $sql = $pdo->prepare("SELECT p.nome, l.titulo, i.* FROM item i 
          ...
          JOIN lista_compras l ON i.lista_id = l.id WHERE nome LIKE :nome ORDER BY id ASC");
    ...
    $sql = $pdo->prepare("SELECT p.nome, l.titulo, i.* FROM item i 
        JOIN produto p ON i.produto_id = p.id
        JOIN lista_compras l ON i.lista_id = l.id ORDER BY id ASC");
...
<form method="post" action="?page=item">
  <div class="container">
  ...
  ?>
    ...
    <tbody>
      <tr>
        ...
        <td>
          <form method="post" action="?page=editar_item">
        ...
        <td>
          <form method="post" action="?page=servico_item">
            <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
